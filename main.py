...
c = {
    '#': "#FF0000",  # Merah
    ' ': "#FFFFFF"   # Putih (gunakan spasi untuk putih)
}

# Function to log messages with a header and timestamp
def log_header():
    print("========================================")
    print("=         NotPixel - UGD AIRDROP       =")
    print("=       Channel : t.me/ugdairdrop      =")
    print("========================================")

# Function to log messages with timestamp and severity level
def log_message(message, level="INFO", color=Style.RESET_ALL):
    ...
    log_color = level_colors.get(level, Style.RESET_ALL)
    print(f"{log_color}{current_time} [{level}] {color}{message}{Style.RESET_ALL}")
...
def get_color(pixel, header):
    try:
        ...
        log_message("Request timed out", "ERROR", Fore.RED)
        return "#000000"
    except requests.exceptions.ConnectionError as e:
        log_message(f"Connection error: {e}", "ERROR", Fore.RED)
        return "#000000"
    except requests.exceptions.RequestException as e:
        log_message(f"Request failed: {e}", "ERROR", Fore.RED)
        return "#000000"

# Function to claim resources from the server
def claim(header):
    log_message("Claiming resources", "INFO", Fore.CYAN)
    try:
        session.get(f"{url}/mining/claim", headers=header, timeout=10)
    except requests.exceptions.RequestException as e:
        log_message(f"Failed to claim resources: {e}", "ERROR", Fore.RED)
...
def paint(canvas_pos, color, header):
    ...
    try:
        ...
        if response.status_code == 400:
            log_message("Out of energy", "ERROR", Fore.RED)
            return False
        if response.status_code == 401:
            return -1

        log_message(f"Paint: {x},{y}", "INFO", Fore.GREEN)
        return True
    except requests.exceptions.RequestException as e:
        log_message(f"Failed to paint: {e}", "ERROR", Fore.RED)
        return False
...
def fetch_mining_data(header):
    try:
        ...
        if response.status_code == 200:
            ...
            log_message(f"Balance: {user_balance}", "INFO", Fore.MAGENTA)
        else:
            log_message(f"Failed to fetch mining data: {response.status_code}", "ERROR", Fore.RED)
    except requests.exceptions.RequestException as e:
        log_message(f"Error fetching mining data: {e}", "ERROR", Fore.RED)
    ...
    try:
        ...
        for pos_image in order:
            ...
            try:
                ...
                if color == -1:
                    log_message("DEAD :(", "ERROR", Fore.RED)
                    print(headers["authorization"])
                    break

                if image[y][x] == ' ' or color == c[image[y][x]]:
                    log_message(f"Skip: {start_x + x - 1},{start_y + y - 1}", "WARNING", Fore.RED)
                    continue
                ...
                log_message(f"IndexError at pos_image: {pos_image}, y: {y}, x: {x}", "ERROR", Fore.RED)

    except requests.exceptions.RequestException as e:
        log_message(f"Network error in account {account}: {e}", "ERROR", Fore.RED)
...
def process_accounts(accounts):
    ...
    for account in accounts:
        ...
        log_message(f"--- Username: {username} ---", "INFO", Fore.BLUE)
    ...
    if time_to_wait.total_seconds() > 0:
        log_message(f"SILAHKAN TUNGGU {int(time_to_wait.total_seconds() // 60)} MENIT", "INFO", Fore.YELLOW)
        time.sleep(time_to_wait.total_seconds())
    else:
        log_message(f"TIDAK PERLU MENUNGGU, UNTUK MEMPROSES MEMBUTUHKAN SEKITAR 1 JAM", "INFO", Fore.YELLOW)
...
if __name__ == "__main__":
    ...
    try:
        ...
        log_message("Process interrupted by user.", "WARNING", Fore.YELLOW)
